...
class PurePursuit(Node):
    ...
    def pose_callback(self, odometry_msg):
        ...
        vectors = ends - starts # vectors from each poitn to the next one
        ...
        closest_segment_index = np.argmin(distances[:self.recent_point+10])
        ...
        # for i in range(closest_segment_index, self.recent_point+5):
            P1 = starts[i]
            V = vectors[i]
            a = np.dot(V, V)
