...
class LaneSegmentation(Node):
    ...
    """
    def __init__(self):
        super().__init__("lane_segmentation_old")
    ...
    def image_callback(self, image_msg):
        ...
        # avg_slope = get_line_slope(image, last=self.last_goal)

        # if avg_slope is not None:
        #     self.get_logger().info("Slope: " + str(avg_slope))
        #     output_point = Point()
        #     output_point.x = float(avg_slope)
        #     output_point.y = 0.0
        #     self.last_goal = output_point.x
...
def main(args=None):
    ...
    lane_segmentatio_old = LaneSegmentation()
